...
The second command selects an automation environment; use a separate UV_PROJECT_ENVIRONMENT when keeping application and tool environments side by side. `giton` itself is excluded from the registry catalog when it is the local editable package.
...
The root pytest configuration also exposes the example project source directory, so the default run collects both application and example tests without an import error.
